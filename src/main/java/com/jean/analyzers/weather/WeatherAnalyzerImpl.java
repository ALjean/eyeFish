...
public class WeatherAnalyzerImpl implements WeatherAnalyzer {
	...
	public double StabilityChecker(float[] temperatureParams) {
		...
		float generalChange = Math
				.abs((temperatureParams[temperatureParams.length - 1] - temperatureParams[0]));
		...
		for (int i = 0; i < temperatureParams.length - 1; i++) {
			 ...
			 */
			float dayChange = Math.abs(temperatureParams[i + 1]
					- temperatureParams[i]);
			...
			if (dayChange == 0) {
				...
				result += dayChange
						* ConstantsAnalyzer.PERCENT_TEMPERATURE_CHANGES;
			}
	...
	public double windChecker(double temperature, double degrees, double speed) {
		...
		WindDirectionConverter windConverter = WindDirectionConverter
				.getInstance();
		...
		if (speed > bofortScale.get(5).getStartValue()) {
			result = 6.25;
		} else if (speed >= bofortScale.get(0).getStartValue()
				&& speed <= bofortScale.get(2).getEndValue()) {
			result = 0.0;
		} else {

			DirectionMap directionMap = WindDirectionConverter.getInstance()
					.getDirectionObject(degrees);

			String direction = directionMap.getDirection();

			if (temperature > ConstantsAnalyzer.CRITICAL_HIGH_TEMPERATURE
					&& (direction.equals("N") | direction.equalsIgnoreCase("NNE") | direction.equalsIgnoreCase("NE") | direction.equalsIgnoreCase("NNW"))) {

				if (speed > bofortScale.get(3).getStartValue()
						&& speed < bofortScale.get(3).getEndValue()) {
					result = 70.23;
				}
				if (speed > bofortScale.get(4).getStartValue()
						&& speed < bofortScale.get(4).getEndValue()) {
					result = 80.52;
				}
				if (speed > bofortScale.get(5).getStartValue()
						&& speed < bofortScale.get(5).getEndValue()) {
					result = 90.52;
				}
			} else {
	...
	public double pressureChecker(float[] pressureParams) {
		...
		float generalChange = pressureParams[pressureParams.length - 1]
				- pressureParams[0];
		...
		if (generalChange == 0 || generalChange > 0) {
			return ConstantsAnalyzer.PERCENT_PRESSURE_CHANGES
					* ConstantsAnalyzer.CRITICAL_PRESSURE_CHANGES;
		}
		if (generalChange < 0) {
			return Math.abs(Math.abs(generalChange)
					* ConstantsAnalyzer.PERCENT_PRESSURE_CHANGES
					/ pressureParams.length - 100);
		}
	...
	@Override
	public double badWeatherAnalyzer(double rain, double clouds,
			double temperature) {
	...
	public static void main(String[] args) {
		...
		System.out.println("Stability of weather are: "
				+ analyzer.StabilityChecker(temperatureParams));

		System.out.println("Pressure activity are: "
				+ analyzer.pressureChecker(pressureParams));

		System.out.println("Wind activity is "
				+ analyzer.windChecker(32, 348, 5.6));
