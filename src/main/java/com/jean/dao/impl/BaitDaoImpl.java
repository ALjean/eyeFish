...
public class BaitDaoImpl extends BaseDaoImpl implements BaitDao {
	...
	public List<Bait> getBaitsToFishesByDate(int fishId, Date date) throws DaoDfmException {
		...
		"SELECT b.bait_id, b.bait_name, bt.bait_type_name, bt.bait_type_id, b.description " + "FROM " + "baits AS b "
				+ "INNER JOIN" + " bait_types AS bt ON bt.bait_type_id = b.bait_type_id " + "INNER JOIN "
				+ "baits_to_fishes AS btf ON btf.bait_id = b.bait_id " + "INNER JOIN "
				+ "baits_to_seasons AS bts ON bts.bait_id = b.bait_id "
				+ "WHERE ? BETWEEN bts.start_period AND bts.end_period AND btf.fish_id = ?";

		List<Bait> baits = new ArrayList<>();

		log.info("Starting method getBaitsForFishByDate(), with parameter values: [ fishId: " + fishId + ", date: "
				+ date + " ]");
	...
	public Integer saveBait(Bait bait) throws DaoDfmException, CustomDfmException {
		...
		String sqlInsertBaitSettings = "INSERT INTO bait_settings bs (bs.bait_id, bs.setting_name, bs.setting_type, bs.description) VALUES (?, ?, ?, ?)";
		String sqlInsertQulifers = "INSERT INTO qualifiers AS q (q.param_name, q.setting_id, q.min_level, q.max_level) VALUES (?, ?, ?, ?)";
	...
	public Bait getBait(int baitId) throws DaoDfmException, CustomDfmException {
		...
		"SELECT b.bait_id, b.bait_name, bt.bait_type_id, bt.bait_type_name, b.description " + "FROM " + "baits AS b "
				+ "INNER JOIN " + "bait_types AS bt ON b.bait_type_id = bt.bait_type_id " + "WHERE b.bait_id = ?";
	...
	public List<Bait> getBaitsToFishes(int fishId) throws DaoDfmException {
		...
		"SELECT b.bait_id, b.bait_name, bt.bait_type_id, bt.bait_type_name, b.description " + "FROM " + "baits AS b "
				+ "INNER JOIN " + "bait_types AS bt ON b.bait_type_id = bt.bait_type_id " + "INNER JOIN "
				+ "baits_to_fishes AS bts ON bts.bait_id = b.bait_id " + "WHERE bts.fish_id = ?";
	...
	public List<Bait> getBaits() throws DaoDfmException, CustomDfmException {
		...
		"SELECT b.bait_id, b.bait_name, bt.bait_type_id, bt.bait_type_name, b.description " + "FROM " + "baits AS b "
				+ "INNER JOIN " + "bait_types AS bt ON b.bait_type_id = bt.bait_type_id ";
	...
	private void setQulifierStatement(int settingId, Qualifier qualifier, PreparedStatement statement)
			throws SQLException {
		statement.setString(1, qualifier.getParameterName().getParamName());
		statement.setInt(2, settingId);
