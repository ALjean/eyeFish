...
public class BaitDaoImpl extends BaseDaoImpl implements BaitDao {
	...
	@Override
	public List<Bait> getBaitsByPondParams(String baitType, String baitName, Integer fishId,
			PondEnvirmoment pondEnv) throws DaoDfmException {
		...
		String sql =
				...
				"SELECT DISTINCT b.bait_id, b.bait_type, b.bait_name, b.description, bs.setting_id, bs.setting_name, bs.setting_type, bs.description, q.qual_id, q.param_name, q.min_level, q.max_level "
						...
						+ "qualifiers AS q " 
						+ "INNER JOIN "
						+ "bait_settings AS bs ON bs.setting_id = q.setting_id " 
						+ "INNER JOIN "
						...
						+ "AND " 
						+ "(b.bait_type LIKE ? OR ? IS NULL) " 
						+ "AND "
						+ "(b.bait_name LIKE ? OR ? IS NULL) " 
						+ "AND "
						+ "((? BETWEEN b_seas.start_period AND b_seas.end_period) OR ? IS NULL)";
		...
		try {
			...
			Log.startDaoLog("getBaitsByPondParams", "fishId: " + fishId + ", baitType: " + baitType + ", baitName: "
					+ baitName + ", date: " + pondEnv.getDate() + ", " + pondEnv.toString());

			preparedStatement.setFloat(1, pondEnv.getCloudLevel());
			preparedStatement.setFloat(2, pondEnv.getDeepLevel());
			preparedStatement.setFloat(3, pondEnv.getTemp());
			preparedStatement.setFloat(4, pondEnv.getAlgalevel());
			preparedStatement.setFloat(5, pondEnv.getRainLevel());
			preparedStatement.setFloat(6, pondEnv.getWindSpeed());
			preparedStatement.setFloat(7, pondEnv.getPressure());

			if (fishId != null) {
				preparedStatement.setFloat(8, fishId);
				preparedStatement.setFloat(9, fishId);
			} else {
			...
			if (!StringUtils.isEmpty(baitType)) {
				preparedStatement.setString(10, baitType.replace("*", "%"));
				preparedStatement.setString(11, baitType.replace("*", "%"));
			} else {
				preparedStatement.setNull(10, Types.CHAR);
			...
			if (!StringUtils.isEmpty(baitName)) {
				preparedStatement.setString(12, baitName.replace("*", "%"));
			...
			if (pondEnv.getDate() != null) {
				preparedStatement.setDate(14, pondEnv.getDate());
				preparedStatement.setDate(15, pondEnv.getDate());
			} else {
