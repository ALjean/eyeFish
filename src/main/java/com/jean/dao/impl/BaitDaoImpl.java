...
public class BaitDaoImpl extends BaseDaoImpl implements BaitDao {
	...
	@Override
	public List<Bait> getBaits(Integer baitId, String baitName, Integer fishId, Date date)
		...
		try {
			...
			if (fishId != null) {
				preparedStatement.setDouble(1, fishId);
				preparedStatement.setDouble(2, fishId);
			} else {
	...
	@Override
	public List<Bait> getBaitsByPondParams(Integer baitId, String baitName, Date date, Integer fishId,
			PondEnvirmoment pondEnv) throws DaoDfmException, CustomDfmException {
		...
		String sql =
				...
				"SELECT DISTINCT b.bait_id, b.bait_name, b.description, bs.setting_id, bs.setting_name, bs.setting_type, bs.description, q.qual_id, q.param_name, q.min_level, q.max_level "
						...
						+ "q.param_name = 'CLOUD_LEVEL' AND (? BETWEEN q.min_level AND q.max_level) " + "OR "
						+ "q.param_name = 'DEEP_LEVEL' AND (? BETWEEN q.min_level AND q.max_level) " + "OR "
						+ "q.param_name = 'ENVIRMOMENT_TEMPERATURE' AND (? BETWEEN q.min_level AND q.max_level) "
						+ "OR " + "q.param_name = 'WATER_TEMPERATURE' AND (? BETWEEN q.min_level AND q.max_level) "
						+ "OR " + "q.param_name = 'ALGA_LEVEL' AND (? BETWEEN q.min_level AND q.max_level) " + "OR "
						+ "q.param_name = 'RAIN_LEVEL' AND (? BETWEEN q.min_level AND q.max_level) " + "OR "
		...
		try {
			...
			Log.startDaoLog("getBaitsByPondParams", "fishId: " + fishId + ", baitId: " + baitId + ", baitName: "
					+ baitName + ", date: " + date + ", " + pondEnv.toString());
	...
	public void updateBait(Bait bait) throws DaoDfmException, CustomDfmException {
		...
		String sqlInsertBaits = "UPDATE baits SET bait_name = ?, description = ? WHERE bait_id = ?";
	...
	@Override
	public Integer deleteBait(List<Integer> listId) throws DaoDfmException, CustomDfmException {
